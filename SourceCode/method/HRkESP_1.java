	...
	public static Result execute(ArrayList<Integer>[] BSGraph, ArrayList<Integer>[] access, int[][] weight) {
		...
		while (servers.size() < ConstNum.nServer) {
			...
			if (nearBaseStaions.isEmpty()) {
				...
				// Find a suitable base station as candidate server deployment location 
				// from the remaining base stations. (robustness + coverage/UserNum)
				int tmpID1 = -1;
				double tmpR1 = 0;
				for (int i = 0; i < weight.length; i++) {
					if (servers.contains(i) || nearBaseStaions.contains(i))
						continue;
					for (int j = i + 1; j < weight[i].length; j++) {
						if (servers.contains(j) || nearBaseStaions.contains(j))
							continue;
						double tmp = weight[i][j] + 1.0 * weight[j][j] / ConstNum.nUser;
						if (tmpR1 < tmp) {
							tmpID1 = j;
							tmpR1 = tmp;
						}
				...
				tmpR1 /= 2; // Achieving robustness increase need two servers.

				// Find a suitable base station as candidate server deployment location 
				// from the neighboring base stations. (robustness + coverage/UserNum)
				int tmpID2 = nearBaseStaions.get(0);
				int tmpR2 = 0;
				for (int j = 0; j < servers.size(); j++) {
					if (BSGraph[tmpID2].contains(servers.get(j))) {
						tmpR2 += weight[tmpID2][servers.get(j)];
					}
				}

				// Compare two candidate base station and choose the highest robustness ones.
				if (tmpR1 > tmpR2) {
					nextServerID = tmpID1;
				} else {
					nextServerID = tmpID2;
